...
;; Version: $Id: lookup-types.el,v 1.1.1.1.4.18 2011-06-08 12:57:11 kazuhiro Exp $
...
circumstances."
    (and lookup-inline-image
	 ...
	 (or (memq type (image-instantiator-format-list))
	     (and  (memq type '(pgm ppm))
		   (memq 'pbm (image-instantiator-format-list))))))
...
the present circumstances. TYPE is a symbol like `xbm' or `jpeg'."
  ...
  (defun lookup-glyph-paste (start end glyph)
    (when (and (featurep 'meadow) (null (stringp glyph)))
      ;; On Meadow, we need to explicitly indicate foreground and
      ;; background colors of xbm image.  Beacuse xbm image is not
      ;; colored according to text's face on Meadow.
      (let* ((face (or (get-text-property start 'face)
		       'default))
	     (fg (face-foreground face))
	     (bg (face-background face)))
	(when fg (setq glyph (append glyph `(:foreground ,fg))))
	(when bg (setq glyph (append glyph `(:background ,bg))))))
    (add-text-properties
  ...
  (defun lookup-img-file-insert (file type &optional start end)
    (when (or (not ndeb-max-image-size)
