...
(defconst navi2ch-article-ident
  "$Id: navi2ch-article.el,v 1.298.2.3 2009/02/25 17:00:29 nawota Exp $")
...
(defun navi2ch-article-goto-number (num &optional save pop)
  ...
  (when (and num (> num 0)
	     navi2ch-article-message-list)
    (when (or (called-interactively-p) save)
	...
	(unless (or navi2ch-article-hide-mode 
	  ...
	  (if navi2ch-article-redraw-when-goto-number
		...
		(navi2ch-article-fix-range num)
		(navi2ch-article-redraw))
	    (if (or (called-interactively-p) pop)
		(progn (when (or (called-interactively-p) save)
...
呼び出す際は INTERACTIVE-FLAG に (called-interactively-p) を入れる。"
  (interactive)
  (or num (setq num 1))
  (if (and (not (eq num 1))
	   (not (eq num -1)))
...
(defun navi2ch-article-through-next ()
  "次のスレに移動する。"
  (interactive)
  (navi2ch-article-through-subr (called-interactively-p) 1))
  ...
  (let* ((article navi2ch-article-current-article)
    ...
    (setq file
	  (if (or (not dir-or-file)
		  (and dir (called-interactively-p)))
	      (expand-file-name
