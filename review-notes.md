...
### dired のファイラ化は全体が仮仕様 (2026-08-10)

**目的は Emacs を OS 標準ファイラの代わりに使うこと。** 2026-08-08〜10 に入れた
一式 (`inits/my-utils/my-dired-{mouse,tabs,sidebar,view,open}.el` と
`inits/05_dired.el` の該当部分) は、**実際に使ってみないと良し悪しが判断できない
たぐいのもの**なので、確定仕様として扱わない。特に次の 2 つ:

- **クリック時の挙動** — 左クリックで入る/開く、行頭のマーク桁でマーク、
  ▶ で展開、中クリックで新しいタブ、Alt+クリックでマーク、サイドボタンで戻る/進む
- **拡張子ごとの開き先** — 動画・画像・PDF・Office は OS 既定アプリ、
  html/md は Vivaldi、それ以外は Emacs。打ち消しは `v` と C-u 前置

**今後、実運用で不足している機能と使いづらい点を洗い出してフィードバックする予定。**
それを受けて仕様を変える前提なので、ここに挙げた挙動は「決まったこと」ではない。
現時点で確定しているのは**目的**(ファイラ置き換え)と**キー割り当ての前提**
(§5 の入力環境。身体的制約が根拠なので動かさない)だけ。
