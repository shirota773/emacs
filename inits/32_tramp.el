;;; 32_tramp.el --- TRAMP (リモート編集) 設定 -*- lexical-binding: t; -*-

;;; Commentary:
;; 30_test-new.el の「2. TRAMP チューニング」を切り出したもの (2026-07-28)。
;; 実機検証を終えて定着したため独立させた。
;;
;; 節番号は 2-1 〜 2-6 のまま残してある。inits/README.md と review-notes.md が
;; この番号で参照しており、振り直すと記録側の指す先が消えるだけになるため。
;;
;;   2-1.   Windows のみ: ホスト単位の ssh ハング回避
;;   2-2.   ホスト補完のソースを絞り、環境固有ホストは外から注入する
;;   2-3.   接続キャッシュ由来の候補を user@host の 1 候補にまとめる
...
;;     ControlPersist 10m
;;
;; ロード順: 32 は 02_completion.el (consult / consult-dir / embark) や
;; 04_bufferlo.el (tab-dir-util の consult-dir ソース) より後になる。ただし
;; ここでの登録はすべて with-eval-after-load か after-init-hook 経由なので
;; 順序に依存しない。
...
;; 先頭に 2 本足しているが、あちらは 04_bufferlo.el 経由で先に読まれる。
;; init-loader はファイル名順なので 32_ はどちらより後で、順序の心配は要らない。
