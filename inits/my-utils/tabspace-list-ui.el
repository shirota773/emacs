;;; tabspace-list-ui.el --- tabspace の対話的リスト UI -*- lexical-binding: t; -*-

;;; Commentary:
;; tabspace-util.el から分離した対話的 UI 部分。
;;   - *Tab Sessions*      : 保存済みセッションのダッシュボード (起動画面)
;;   - *Tabspaces Buffers* : タブ×バッファの ibuffer 風一覧
;; セッションの読み書きなどコア機能は tabspace-util.el に依存する。
...
(defun my/tab-util--insert-ascii-header (subtitle)
  "ASCII アートのヘッダーと SUBTITLE を挿入する。"
  (let ((inhibit-read-only t))
    (dolist (line '("  _____      _                               \n"
                    " |_   _|_ _ | |__  ___ _ __   __ _  ___ ___ \n"
                    "   | |/ _` || '_ \\/ __| '_ \\ / _` |/ __/ _ \\\n"
                    "   | | (_| || |_) \\__ \\ |_) | (_| | (_|  __/\n"
                    "   |_|\\__,_||_.__/|___/ .__/ \\__,_|\\___\\___|\n"
                    "                      |_|                    \n"))
      (insert (propertize line 'face 'my/tabspace-session-header)))
    (insert "\n")
    (insert (propertize subtitle 'face 'my/tabspace-session-meta))
...
;; =============================================================================
;; 2. Tab Session List Mode Implementation (Dashboard-like)
;; =============================================================================
...
(defun my/tabspace-session-list-refresh ()
  ...
  (let ((inhibit-read-only t)
        (saved-point (point)))
    (erase-buffer)
    (my/tab-util--insert-ascii-header "  Manage and Restore your Saved Tabspaces\n")
    (insert (propertize
...
;; =============================================================================
;; 3. Tabspace Buffer List (ibuffer-like) Implementation
;; =============================================================================
...
(defun my/tabspaces-refresh-buffer-list ()
  (interactive)
  (let ((inhibit-read-only t) (saved-point (point)) (i 1))
    (erase-buffer)
    (my/tab-util--insert-ascii-header "  Manage your Tabspaces and active Buffers\n")
...
(provide 'tabspace-list-ui)

;;; tabspace-list-ui.el ends here
